...
export default function Dashboard(){
    ...
    return <div>
        ...
        <Bar data={{
            ...
            datasets:[{label:"Clicks",data:barData.map(d=>d.count)}]
        }} 
        onClick={(e,elem)=>{
            if(elem.length>0){
                const idx = elem[0].index;
                setSelectedFeature(barData[idx].featureName);
                trackClick(barData[idx].featureName);
            }
        }} />

        {selectedFeature && <Line data={{
            labels: ["Day1","Day2","Day3"], 
            datasets:[{label:selectedFeature,data:[5,3,2]}]
        }} />}
    </div>
}
