...
export default function Dashboard() {
  ...
  const [filters, setFilters] = useState({
    startDate: savedFilters.startDate ? new Date(savedFilters.startDate) : new Date(new Date().setDate(new Date().getDate() - 7)),
    ...
    <div style={{ padding: "20px" }}>
      ...
      <div style={{ maxWidth: "700px", marginBottom: "40px" }}>
        <Bar
          ...
          data={{
            ...
            datasets: [{ label: "Total Clicks", data: barData.map((d) => d.count), backgroundColor: "orange" }],
          }}
      ...
      {selectedFeature && (
        <div style={{ maxWidth: "700px" }}>
          <Line
            key={selectedFeature + JSON.stringify(lineData)}
            data={lineData}
          />
        </div>
      )}
