...
export default function Dashboard() {
  ...
  const fetchBarData = async () => {
    ...
    try {
      ...
      setBarData(
        data.map((item) => ({
          featureName: item.featureName || item[0],
  ...
  // ===== Track clicks to backend safely =====
  const trackClick = async (featureName) => {
    const userId = localStorage.getItem("userId");
    if (!userId) {
      console.warn("No userId found; skipping click tracking");
      return;
    }
    try {
      ...
      console.error("Failed to track click", err.response?.data || err);
    }
  };
