...
export default function Dashboard() {
  ...
  const fetchAnalytics = async () => {
    try {
      ...
      // bar chart data
      const barArr = Object.entries(res.data.barData).map(([feature, count]) => ({ feature, count }));
      setBarData(barArr);
      // line chart data (per feature map)
      setLineData([]); // reset line chart until user clicks a bar
      window.lineDataMap = res.data.lineData; // save globally for prepareLineChart
