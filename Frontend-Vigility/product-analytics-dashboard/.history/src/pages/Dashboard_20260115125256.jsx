...
export default function Dashboard() {
  ...
  const fetchAnalytics = async () => {
    try {
      ...
      if (selectedFeature && res.data.lineData[selectedFeature]) {
        ...
        if (keys.length) {
          // merge line data by date
          const allDatesSet = new Set();
          keys.forEach(f => {
            res.data.lineData[f].forEach(pt => allDatesSet.add(pt.date));
          });
          ...
          allDates.forEach(date => {
            const point = { date };
            keys.forEach(f => {
              const found = res.data.lineData[f].find(pt => pt.date === date);
              point[f] = found ? found.count : 0;
            });
            combined.push(point);
          });
        }
  ...
  const trackClick = async (featureName) => {
    try {
      ...
      fetchAnalytics(); // refresh charts after click
  ...
  const setQuickRange = (range) => {
    let start, end;
    switch (range) {
      case "today": start = end = dayjs(); break;
      case "yesterday": start = end = dayjs().subtract(1, "day"); break;
      case "last7": start = dayjs().subtract(6, "day"); end = dayjs(); break;
      case "month": start = dayjs().startOf("month"); end = dayjs().endOf("month"); break;
      default: return;
    }
