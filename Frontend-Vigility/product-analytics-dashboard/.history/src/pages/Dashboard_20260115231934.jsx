...
export default function Dashboard() {
  ...
  const fetchAnalytics = async () => {
    try {
      ...
      // Features to show in dashboard as per document
      const dashboardFeatures = ["date_picker", "filter_age", "chart_bar", "filter_gender"];

      // Bar Chart data
      const bars = Object.entries(res.data.barData || {})
        .filter(([feature]) => dashboardFeatures.includes(feature))
        .map(([feature, count]) => ({ feature, count }));
      setBarData(bars);

      // Line Chart data
      if (selectedFeature && dashboardFeatures.includes(selectedFeature)) {
        setLineData(res.data.lineData?.[selectedFeature] || []);
      } else {
        setLineData([]); // default empty
      }
