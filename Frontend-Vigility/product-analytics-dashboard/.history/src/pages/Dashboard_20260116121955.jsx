...
//       // ---------------- LINE CHART (TOTAL CLICKS DATE-WISE) ----------------
...
//   useEffect(() => {
//     fetchAnalytics();
//   }, [filters.age, filters.gender, dateRange[0], dateRange[1]]);
...
export default function Dashboard() {
  ...
  const fetchAnalytics = async () => {
    try {
      ...
      // ---------------- LINE CHART ----------------
      if (selectedFeature) {
        // LEFT chart pe click → selected feature ka daily clicks
        ...
        // Default: sab 4 features ka combined daily clicks
        const datesSet = new Set();
        dashboardFeatures.forEach(f => {
          Object.keys(res.data.lineData || {}).forEach(k => {
            if (k.startsWith(f)) res.data.lineData[k].forEach(d => datesSet.add(d.date));
          });
        });

        const combined = [...datesSet].sort().map(date => {
          let rowTotal = 0;
          dashboardFeatures.forEach(f => {
            Object.keys(res.data.lineData || {}).forEach(k => {
              if (k.startsWith(f)) {
                const found = res.data.lineData[k].find(x => x.date === date);
                if (found) rowTotal += found.count;
              }
            });
          });
          return { date, clicks: rowTotal };
        });
