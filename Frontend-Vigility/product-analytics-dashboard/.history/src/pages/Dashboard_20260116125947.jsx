...
export default function Dashboard() {
  ...
  useEffect(() => { fetchAnalytics(); }, [filters.age, filters.gender, dateRange[0], dateRange[1], selectedFeature]);
  ...
  const handleQuickDate = (option) => {
    ...
    switch(option){
      ...
      case "thisMonth": start = dayjs().startOf("month"); break;
      case "reset":
        start = dayjs().subtract(7,"day"); end = dayjs();
        setFilters({ age:"", gender:""});
        Cookies.remove("age"); Cookies.remove("gender");
        break;
      default: return;
  ...
  return (
    <Container maxWidth="lg" sx={{ py:4 }}>
      <Box sx={{ display:"flex", flexDirection:{ xs:"column", sm:"row" }, justifyContent:"space-between", alignItems:{ xs:"flex-start", sm:"center" }, mb:3 }}>
        <Typography variant="h4" mb={{ xs:2, sm:0 }}>📊 Product Analytics Dashboard</Typography>
        <Button color="error" variant="contained" onClick={handleLogout}>Logout</Button>
      </Box>
