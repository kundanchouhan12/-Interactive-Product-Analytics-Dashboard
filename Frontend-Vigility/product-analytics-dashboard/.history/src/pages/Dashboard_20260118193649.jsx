...
export default function Dashboard() {
  ...
  const fetchAnalytics = async () => {
    try {
      ...
      // FIXED: barData
      const bars = Object.entries(res.data.barData || {}).map(([key, value]) => ({
        feature: key.replaceAll("_", " "), // display friendly
        count: value,
        rawKey: key, // for line chart
      }));
      setBarData(bars);

      // FIXED: lineData for selectedFeature
      const keys = selectedFeature
        ? [selectedFeature]
    ...
    <Container maxWidth="lg" sx={{ py: 4 }}>
      ...
      <Box display="flex" justifyContent="space-between" alignItems="center" flexWrap="wrap" gap={2} mb={3}>
        ...
        <Box>
          ...
          <Button color="error" variant="contained" sx={{ ml: 2, borderRadius: 3 }} onClick={handleLogout}>
            Logout
          </Button>
        </Box>
      </Box>
      ...
      <Card sx={{ mb: 4, borderRadius: 4 }}>
        <CardContent>
          ...
          <Box display="flex" gap={2} rowGap={1.5} flexWrap="wrap" mb={2}>
            {["today", "yesterday", "last7", "thisMonth", "custom", "reset"].map((k) => (
              <Button
                key={k}
                size="small"
                variant={k === "custom" ? "contained" : "outlined"}
          ...
          {customOpen && (
            <LocalizationProvider dateAdapter={AdapterDayjs}>
              <Box display="flex" gap={2} flexWrap="wrap" mb={3}>
                <DatePicker label="Start Date" value={dateRange[0]} onChange={(v) => setDateRange([v, dateRange[1]])} />
                <DatePicker label="End Date" value={dateRange[1]} onChange={(v) => setDateRange([dateRange[0], v])} />
                <Button variant="contained" color="success" onClick={applyCustomRange}>
                  Apply
                </Button>
                <Button variant="outlined" color="error" onClick={() => setCustomOpen(false)}>
                  Cancel
                </Button>
              </Box>
            </LocalizationProvider>
